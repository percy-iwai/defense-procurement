...
_KEYWORD_RULES_RAW: list[tuple] = [
    ...
    (["イージス", "統合防空", "PAC-3", "SM-3", "SM-6", "ペトリオット",
      ...
      "警戒管制レーダ", "地対空誘導弾", "高出力レーザ",
      "防空ミサイル",
      "E-2D", "E2D", "早期警戒機"],
     2, None, 0.82),
    # ※ HPM・高出力マイクロ波 は P43（電磁波）ルールへ移管（→ P43 0.82 ブロック参照）
    ...
    # ── P82 次期戦闘機の開発（P43「次期戦闘機」0.88 に勝つ conf=0.90）───────────
    ...
    # ── P43 電磁波兵器（HPM・高出力マイクロ波: P2から移管、conf=0.82）────────────
    # 根拠: HPM装置(検証用)→P43確定。高出力マイクロ波・電磁波兵器は電磁波ドメイン
    # HPM・高出力マイクロ波を P2 ルールから削除し本ルールへ集約
