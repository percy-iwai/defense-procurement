"""Check PDF contents of chuou_chotatsu_jisseki files to verify fiscal year."""
import pdfplumber
import re
import os
import sys
...
def process_dir(dirpath, label):
    ...
    for fname in files:
        ...
        try:
            ...
            heisei_str = ",".join(sorted(set(heisei), key=int)) if heisei else "-"
            print(f"\nFILE: {fname}")
            print(f"  平成年度: {heisei_str}")
            print(f"  令和年度: {reiwa_str}")
            print(f"  実績: {jisseki}, 見込: {mikomi}")
            snippet = text[:500].replace("\n", " / ").strip()
            print(f"  先頭テキスト: {snippet[:400]}")
